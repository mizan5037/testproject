...
if(isset($_GET['fab_rec_id']) && $_GET['fab_rec_id'] != ''){
    $buyer_id = mysqli_real_escape_string($conn, $_GET['fab_rec_id']);
    $color    = mysqli_real_escape_string($conn, $_GET['color']);
    $style    = mysqli_real_escape_string($conn, $_GET['style']);
    $sql      = "SELECT fr.* FROM fab_receive fr LEFT JOIN masterlc_description md ON fr.POID = md.POID LEFT JOIN (select * from masterlc  WHERE MasterLCBuyer = '$buyer_id' ) m ON md.MasterLCID = m.MasterLCID where fr.Color = '$color' AND fr.StyleID = '$style' GROUP BY fr.FabReceiveID";
    ...
    $buyer_id = mysqli_real_escape_string($conn, $_GET['fab_rec_id_other']);
    $conpoc   = mysqli_real_escape_string($conn, $_GET['conpoc']);
    $color    = mysqli_real_escape_string($conn, $_GET['color']);
