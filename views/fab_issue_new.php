...
if(isset($_GET['fab_issue_id'])){
  $fab_issue_id = $_GET['fab_issue_id'];
}
else {
...
function customPageHeader()
{
    ...
    <!--Arbitrary HTML Tags-->
<?php }
include_once "controller/add_fab_issue_new.php";
        ...
        <div class="card-body">
            ...
            <div class="container">
                <form action="" class="needs-validation" method="POST" novalidate>
                    <div class="form-row">
                      <input type="hidden" name="last_id" value="<?= $fab_issue_id ?>">
                        <?php
                            $sql = "SELECT * FROM `fab_issue` WHERE Status = '1' AND FabIssueID = '$fab_issue_id'";
                            $sql = mysqli_query($conn, $sql);
                            $fab_issue = mysqli_fetch_assoc($sql);
                        ?>
                        <div class="col-md-4">
                            <label for="validationTooltip02">Buyer Name</label>
                            <select name="buyer[]" class="buyer mb-2 form-control-sm form-control" required disabled>
                                <?php
                                $id = $fab_issue['BuyerID'];
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="style[]" class="style mb-2 form-control-sm form-control" required disabled>
                                <?php
                                $id = $fab_issue['StyleID'];
                                $sql = "SELECT * FROM style WHERE status = 1 AND StyleID = '$id'";
                                $results = mysqli_query($conn, $sql);
                                while ($result = mysqli_fetch_assoc($results)) {
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="po[]" class="po mb-2 form-control-sm form-control" required disabled>
                                <?php
                                $id = $fab_issue['POID'];
                                $sql = "SELECT * FROM po WHERE status = 1 AND POID = '$id'";
                                $results = mysqli_query($conn, $sql);
                                while ($result = mysqli_fetch_assoc($results)) {
