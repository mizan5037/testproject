...
$PageTitle = "Carton Update Form | Optima Inventory";
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM carton_form where CartonFromID='$id'";
    $single_carton = mysqli_fetch_assoc(mysqli_query($conn, $sql));
} else {
            ...
            <form class="needs-validation" method="POST" novalidate>
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="date" value="<?= $single_carton['date'] ?>" class="form-control" id="validationTooltip01" placeholder="Style" required>
                    </div>
                </div>
                <div class="form-row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="po" class="po  form-control" required>
                                        ...
                                        while ($result = mysqli_fetch_assoc($results)) {
                                            if ($result['POID']==$single_carton['POID']) {
                                                $selected = 'selected';
                                            }
                                            echo '<option value="' . $result['POID'] . '" selected=".$selected." >' . $result['PONumber'] . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                                <td>
                                    <select name="style" class="style  form-control" required>
                                        ...
                                        $results = mysqli_query($conn, $sql);
                                        while ($result = mysqli_fetch_assoc($results)) {
                                            if ($result['StyleID']==$single_carton['StyleID']) {
                                                $selected = 'selected';
                                            }
                                            echo '<option value="' . $result['StyleID'] . '" selected=".$selected.">' . $result['StyleNumber'] . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                                <td>
                                    <select name="color" class="color  form-control" required>
                                        ...
                                        $results = mysqli_query($conn, $sql);
                                        while ($result = mysqli_fetch_assoc($results)) {
                                            if ($result['id']==$single_carton['color']) {
                                                $selected = 'selected';
                                            }
                                            echo '<option value="' . $result['id'] . '" selected=".$selected.">' . $result['color'] . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
